...
folder are mapped to the application's root namespace, 'KaspiMobile'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "KaspiMobile"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'KaspiMobile' in the same way
as `"KaspiMobile/sass/src"`.
